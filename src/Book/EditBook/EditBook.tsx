...
const EditBook = (props: Props) => {
    ...
    const [actDeactCopyError, setActDeactCopyError] = useState<boolean>(false);

    const [openActDeactCopy, setOpenActDeactCopy] = useState<boolean>(false);
    const [openAsAct, setOpenAsAct] = useState<boolean>(false)
    ...
    const closeActDeactCopy = () => {
        setOpenActDeactCopy(false);
    }

    const onActDeactCopyError = () => {
        notifyError("No se puede cambiar el estado del ejemplar porque se encuentra reservado")
    }
    const onActDeactCopySuccess = (copy:Copy) => {
        updateCopies(copy);
    ...
    return (
        <div className={"edit-book"}>
            ...
            <ActivateDeactivateCopyModal isOpen={openActDeactCopy} onClose={closeActDeactCopy}
                                         copy={selectedCopy}
                                         onSuccess={onActDeactCopySuccess} onError={onActDeactCopyError}
                                         isActive={openAsAct}/>
            <CreateOrEditBook handleCancel={props.handleCancel}
                              ...
                              actDeactCopyError={actDeactCopyError}/>
        </div>
    )
}
